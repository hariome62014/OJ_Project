...
import { fetchProblems, deleteProblem } from '../../../services/operations/ProblemAPI';
...
const AdminPage = () => {
  ...
  const transformProblemData = (problem) => {
    ...
    return {
      ...
      category: problem.tags?.join(', ') || 'Uncategorized',
      tags: problem.tags || [],
  ...
  const filteredProblems = problem
    ...
    .sort((a, b) => {
      switch(sortOption) {
        ...
        case 'published': return a.isPublished === b.isPublished ? 0 : a.isPublished ? -1 : 1;
  ...
  const handleConfirmDelete = async () => {
    if (problemToDelete) {
      try {
        await handleDeleteProblem(problemToDelete);
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'} mt-16`}>
            ...
            >
              ...
              <div className="mb-6">
                ...
                <label className="flex items-center mb-2">
                  <input
                    ...
                    onChange={() => toggleFilter('status', 'published')}
                    className="mr-2"
                  />
                  Published
                </label>
                <label className="flex items-center mb-2">
                  <input
                    type="checkbox"
                    checked={filters.status?.includes('unpublished')}
                    onChange={() => toggleFilter('status', 'unpublished')}
                    className="mr-2"
                  />
                  Unpublished
                </label>
              ...
              <div>
                ...
                >
                  ...
                  <option value="published">Publication Status</option>
                </select>
              </div>
            </motion.div>
          )}
                        ...
                        >
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-sm">
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() => openTestCaseModal(problem)}
                              ...
                              <button
                                onClick={() => openProblemModal(problem, 'edit')}
      ...
      <ConfirmationModal
        ...
        message="This action will leads to the deletion of the selected problem"
        darkMode = {darkMode}
  ...
  previousTestCases={selectedProblem}
  darkMode={darkMode}
/>
      )}
      ...
      {isProblemModalOpen && (
        <ProblemFormModal 
          problem={modalAction === 'edit' ? selectedProblem : null}
